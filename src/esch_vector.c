...
esch_vector_new(esch_config* config, esch_vector** vec)
{
    ...
    ret = esch_alloc_malloc(alloc, sizeof(esch_object*) * initial_length,
                            (void**)&array);
    ESCH_CHECK(ret == ESCH_OK, log, "Failed to get initial length", ret);
    ...
    new_vec->length = (size_t)initial_length;
    new_vec->element_type = element_type;
    new_vec->array = array;
    new_vec->next = &(new_vec->array[0]);
    new_vec->end = (new_vec->array + new_vec->length);
    array = NULL;
    (*vec) = new_vec;
    new_vec = NULL;
Exit:
...
esch_vector_delete(esch_vector* vec, ESCH_BOOL delete_element)
{
    ...
    ret = esch_alloc_free(alloc, vec->array);
...
esch_error esch_list_append(esch_vector* vec, esch_object* data);
